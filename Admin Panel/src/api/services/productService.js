...
export const fetchProducts = async () => {
  ...
  console.log("products for promotion", response.data.data);
...
export const uploadImage = async (file) => {
  ...
  const local = "http://localhost:8000/upload-image";
  ...
  try {
    const response = await axios.post(local, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data",
      },
    });
    console.log("uploaded image response", response.data.data.url);
...
export const productSizeCreate = async (data) => {
  console.log("create size", data);
  const productId = data.productId;
  const response = await axiosInstance.post(
    ENDPOINTS.PRODUCT_SIZE_CREATE,
    data
  );
...
export const updateProductSize = async (id, data) => {
  console.log("update size data to api", id, data);
  const response = await axiosInstance.put(
    ENDPOINTS.PRODUCT_SIZE_UPDATE(id),
    data
  );
  console.log("updated size response", response.data.data);
  return response.data.data;
};
